...
class MPOAgent:
    ...
    def update_actor(self) -> float:
        ...
        for _ in range(self.n_actor_updates):
            ...
            sampled_actions = self.actor.policy.sample(num_samples=self.n_action_proposals)[0]
            ...
            #TODO do we not need to detach the critic here? need to think whether it makes a diff
            # sorted_q = torch.argsort(qvalues, dim=0, descending=True)
            # best_indices = sorted_q[:self.n_action_proposals_selected]
            # best_actions = sampled_actions[best_indices].squeeze()
            ...
            """
            exp_scale = torch.exp((qvalues.squeeze() - self.value_baseline) / self.kl_coef)
            log_probs = self.actor.policy.log_prob(sampled_actions)
            error = - torch.mean(exp_scale * log_probs)
